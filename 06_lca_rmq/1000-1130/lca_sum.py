...
def make_jump_pointers(nodes):
    ...
    while pow_two <= n:
        for i in range(n):
            node = nodes[i]
            if k == 0:
                node.jumps.append(node.parent.label)
            else:
...
def lca(u, v, nodes):
    ...
    return nodes[u].parent.label


def sum_on_tree(u, v, nodes):
    u_node, v_node = nodes[u], nodes[v]
    anc = lca(u, v, nodes)
    anc_node = nodes[anc]
    return u_node.s + v_node.s - (2 * anc_node.s) + anc_node.v
...
if __name__ == '__main__':
    ...
    for U in range(len(adj)):
        for V in range(U + 1, len(adj)):
            ...
            ans = lca(U, V, nodes)
            path_sum = sum_on_tree(U, V, nodes)
            print(f'LCA of {U} and {V} is {ans}, path sum is {path_sum}')
